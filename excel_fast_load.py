...
@author: FelixKling
NOTE: This code is provided as is, with no license, as it was shared by its author:
...
def excel_fast_load(path, sheet_name=None, header=True, index_col=False, skiprows=[], skipcolumns=[]):
    """
    ...
    if sheet_name:
        ...
        if sheet_name in workbooks:
            ...
            raise FileNotFoundError(
                "Sheet " + str(sheet_name) + " not found in Excel file! Available sheets: " + "; ".join(
                    workbooks.keys()))
    ...
    if "xl/sharedStrings.xml" in tempfiles:
        ...
        string_items = re.split(r"<si.*?><t.*?>",
                                text.replace("<t/>", "<t></t>").replace("</t></si>", "").replace("</sst>", ""))[1:]
    ...
    df = df.iloc[
        [i for i in range(len(df)) if i not in skiprows], [i for i in range(len(df.columns)) if i not in skipcolumns]]
